...
-- First, drop policies that depend on the role column to allow type changes
DROP POLICY IF EXISTS "Org admins can manage group sessions" ON public.group_sessions;
DROP POLICY IF EXISTS "Organization admins can view their invoices" ON public.organization_invoices;
...
-- Recreate the dropped policies with corrected references
...
FOR SELECT USING (
  organization_id IN (
    ...
    WHERE id = (SELECT id FROM public.profiles WHERE auth_uid = auth.uid() AND role = 'org_admin')
  )
);
...
BEGIN
    ...
    -- Log the user creation (optional, don't block if this fails)
    BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
        RAISE LOG 'Error logging user creation: %', SQLERRM;
